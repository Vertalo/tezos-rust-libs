...
fn generate_uncompressed_random_element() -> G2Uncompressed {
    let mut random_gen = rand::thread_rng();
    let random_g1 = bls12_381::G2::random(&mut random_gen);
    let uncompressed_form = bls12_381::G2Uncompressed::from_affine(random_g1.into_affine());
    uncompressed_form
}

fn generate_compressed_random_element() -> G2Compressed {
    let mut random_gen = rand::thread_rng();
    let random_g1 = bls12_381::G2::random(&mut random_gen);
...
mod tests {
    ...
    fn bench_uncompressed_generate_one(b: &mut Bencher) {
        b.iter(|| {
            let r = bls12_381::G2Affine::one();
            let uncompressed_form = bls12_381::G2Uncompressed::from_affine(r);
            uncompressed_form
        })
    }
    ...
    fn bench_uncompressed_check_if_zero_on_pregenerated_zero(b: &mut Bencher) {
        let g = bls12_381::G2Uncompressed::from_affine(bls12_381::G2Affine::zero());

        b.iter(|| {
            let g = g.into_affine().unwrap();
            g.is_zero()
        })
    }
    ...
    fn bench_uncompressed_check_if_zero_on_pregenerated_one(b: &mut Bencher) {
        let g = bls12_381::G2Uncompressed::from_affine(bls12_381::G2Affine::one());
    ...
    #[bench]
    fn bench_uncompressed_add(b: &mut Bencher) {
        ...
        b.iter(|| {
            ...
            g1.add_assign(&g2);
            bls12_381::G2Uncompressed::from_affine(g1.into_affine())
        })
    }
    ...
    fn bench_uncompressed_opposite_of_pregenerated_random_element(b: &mut Bencher) {
        ...
        b.iter(|| {
            let mut g = g.into_affine().unwrap();
            g.negate();
            bls12_381::G2Uncompressed::from_affine(g)
        })
    }
    ...
    fn bench_uncompressed_eq_with_pregenerated_random_elements(b: &mut Bencher) {
        ...
        b.iter(|| {
            let g1 = g1.into_affine().unwrap();
            let g2 = g2.into_affine().unwrap();
            g1 == g2
        })
    }
    ...
    fn bench_uncompressed_eq_with_pregenerated_random_element_and_zero(b: &mut Bencher) {
        let g1 = generate_uncompressed_random_element();
        let g2 = bls12_381::G2Uncompressed::from_affine(bls12_381::G2Affine::zero());
    ...
    fn bench_uncompressed_eq_with_pregenerated_random_element_and_one(b: &mut Bencher) {
        let g1 = generate_uncompressed_random_element();
        let g2 = bls12_381::G2Uncompressed::from_affine(bls12_381::G2Affine::one());
    ...
    fn bench_uncompressed_mul_with_pregenerated_random_elements(b: &mut Bencher) {
        ...
        b.iter(|| {
            let mut g = g.into_affine().unwrap().into_projective();
            g.mul_assign(a);
            bls12_381::G2Uncompressed::from_affine(g.into_affine());
        })
    }

    #[bench]
    fn bench_from_uncompressed_to_projective(b: &mut Bencher) {
        let g = generate_uncompressed_random_element();
        b.iter(|| {
    ...
    fn bench_compressed_add(b: &mut Bencher) {
        ...
        b.iter(|| {
            ...
            let g2 = g2.into_affine().unwrap().into_projective();
            g1.add_assign(&g2);
            bls12_381::G2Compressed::from_affine(g1.into_affine())
        })
    }
    ...
    fn bench_compressed_opposite_of_pregenerated_random_element(b: &mut Bencher) {
        ...
        b.iter(|| {
            let mut g = g.into_affine().unwrap();
            g.negate();
            bls12_381::G2Compressed::from_affine(g)
        })
    }

    #[bench]
    fn bench_compressed_negate_of_pregenerated_zero_element(b: &mut Bencher) {
        let g = bls12_381::G2Compressed::from_affine(bls12_381::G2Affine::zero());
    ...
    fn bench_compressed_negate_of_pregenerated_one_element(b: &mut Bencher) {
        let g = bls12_381::G2Compressed::from_affine(bls12_381::G2Affine::one());
    ...
    fn bench_compressed_eq_with_pregenerated_random_element_and_zero(b: &mut Bencher) {
        let g1 = generate_compressed_random_element();
        let g2 = bls12_381::G2Compressed::from_affine(bls12_381::G2Affine::zero());
    ...
    fn bench_compressed_eq_with_pregenerated_random_element_and_one(b: &mut Bencher) {
        let g1 = generate_compressed_random_element();
        let g2 = bls12_381::G2Compressed::from_affine(bls12_381::G2Affine::one());
    ...
    fn bench_compressed_mul_with_pregenerated_random_elements(b: &mut Bencher) {
        ...
        b.iter(|| {
            let mut g = g.into_affine().unwrap().into_projective();
            g.mul_assign(a);
            bls12_381::G2Compressed::from_affine(g.into_affine());
        })
    }
}
